  ...
  return (
    <div className='absolute left-[20%] w-4/5 flex flex-col gap-4'>
      ...
      <AdmnTable
        ...
        TABLE_ROWS={confeds.map(confed =>({id: confed.id, name: confed.confed_name, logo: confed.img_url, alias: confed.confed_3,}))}
        TABLE_LINKS = {confeds.map(({id})=>(`/admin/confederations/edit/${id}`))}
        DELETE_ID={confeds.map(({id})=>id)}
      />
      </div>
    </div>
  )
