...
export default function Edit_country ({ params: { id } }) {
  ...
  const { countryName, countryAbrev, imgUrl, imgWithoutName } = formValues
        ...
        confeds.map(confed =>
          setOptions(options => {
            if (options.label !== confed.confed_3) {
              return [...options, { label: confed.confed_3, value: confed.id }]
            }
          })
        )
    }
  ...
  useEffect(() => {
    async function loadCountry () {
      ...
      setFormValues(prev => ({
        ...
        countryAbrev: country.country_3,
        confedName: country.confederation_id,
        imgUrl: country.country_img_url,
        imgWithoutName: country.country_not_name_img
      }))
      const confed = options.find(
        option => option.value === country.confederation_id
      )
      confed && setActualOPtion(confed)
    }
    loadCountry()
  }, [options])
  useEffect(() => {
    if(imgUrl && images.img) {
      edgestore.publicFiles.delete(({
        url: imgUrl
      }))
      .then(() =>setFormValues(prev => ({
        ...prev,
        imgUrl: ''
      })))
    }
    if(imgWithoutName && images.no_name) {
      edgestore.publicFiles.delete(({
        url: imgWithoutName
      }))
      .then(() =>setFormValues(prev => ({
    ...
    <div className='absolute left-[20%] w-4/5 flex justify-center items-center h-[calc(100vh-62px)]'>
      ...
      >
        ...
        <div className={modalStyle.messages}>
          {state?.message != null && (
            <p aria-live='polite' className={modalStyle.errorMsg} role='status'>
              {state?.message}
            </p>
          )}
        </div>
